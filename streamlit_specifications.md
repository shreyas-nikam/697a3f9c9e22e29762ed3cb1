...
# Streamlit Application Specification: Model Risk Tiering & Control Mandate

## 1. Application Overview

### Purpose of the Application

The **Model Risk Tiering & Control Mandate** Streamlit application serves as a critical Second Line of Defense tool for a **Model Risk Management (MRM) Lead** at a financial institution (BankCo). Its primary purpose is to operationalize key aspects of **SR 11-7 Supervisory Guidance on Model Risk Management**. The app facilitates the ingestion of initial model registration data, the independent calculation of an official model risk tier, the determination of minimum required controls commensurate with that tier, and the generation of auditable decision reports. This workflow ensures consistent, transparent, and reproducible model risk assessment across the enterprise, safeguarding against potential financial, reputational, and regulatory risks.

### High-Level Story Flow

The MRM Lead's journey through the application unfolds as follows:

1.  **Ingest Model Submission**: The MRM Lead begins by uploading a JSON file containing a Model Owner's preliminary registration and self-assessment for a new model. The application validates the structure of this submission and displays key metadata. This simulates receiving a new model into the MRM pipeline.
2.  **Official Tiering**: Upon successful ingestion, the MRM Lead navigates to the "Official Tiering" page. Here, BankCo's standardized, deterministic risk tiering logic (Lab 2 logic) is applied to the submitted model's attributes. The application calculates an official risk score and assigns a definitive Model Risk Tier, showing a transparent breakdown and comparing it against the Model Owner's preliminary assessment. This is where the MRM Lead performs their independent risk assessment.
3.  **Required Controls**: With the official tier established, the MRM Lead proceeds to the "Required Controls" page. The application automatically maps the assigned tier to BankCo's canonical library of control expectations (inspired by SR 11-7). A detailed checklist of mandated controls is presented, categorized for clarity. This step defines the compliance obligations for the model.
4.  **Export Reports**: Finally, on the "Export Reports" page, the MRM Lead reviews a summary of the tiering decision and control mandate. They can add or refine their plain-English rationale for auditability. The application then allows the MRM Lead to download the formal `risk_tiering.json`, `required_controls_checklist.json`, and `executive_summary.md` artifacts, completing the formal model risk assessment process and providing auditable records for internal and regulatory stakeholders.

This flow mirrors a real-world MRM workflow, enabling the MRM Lead to efficiently execute their mandate for model risk governance.
...
# Note: generate_reports from source.py writes to disk and is not directly used for st.download_button.
# The content for download buttons is constructed from session state using json.dumps and string formatting.
```

### `st.session_state` Initialization, Updates, and Reads

The following `st.session_state` keys will be used to maintain application state across user interactions and page navigations:

| Key                              | Initial Value                                 | Description                                                                                                                                                                                                                                                                                                                                                                                                                           |
| :------------------------------- | :-------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `current_page`                   | `"Ingest Model Submission"`                   | Controls the active page displayed in the main content area, based on sidebar selection.                                                                                                                                                                                                                                                                                                                                              |
| `uploaded_file_content`          | `None`                                        | Stores the raw string content of the JSON file uploaded via `st.file_uploader`.                                                                                                                                                                                                                                                                                                                                                       |
| `model_registration_data`        | `None`                                        | Stores the parsed JSON content as a Python dictionary after successful upload and basic schema validation. This object acts as the `model_metadata` for `apply_model_tiering_logic` and `model_metadata_snapshot` for report generation.                                                                                                                                                                                 |
| `validation_status`              | `None`                                        | Boolean indicating the success of the schema validation (`True` for success, `False` for failure). `None` implies no file has been uploaded or processed yet.                                                                                                                                                                                                                                                                     |
| `validation_error`               | `None`                                        | String containing an error message if `validation_status` is `False`, detailing missing keys or parsing issues.                                                                                                                                                                                                                                                                                                                       |
| `tiering_results`                | `None`                                        | Dictionary containing the output from `apply_model_tiering_logic` (e.g., `official_risk_score`, `official_risk_tier`, `score_breakdown`). This is populated after the "Official Tiering" computation.                                                                                                                                                                                                                         |
| `controls_checklist`             | `None`                                        | Dictionary containing the output from `map_controls_to_tier` (e.g., `model_id`, `assigned_tier`, `required_controls`). This is populated after the "Required Controls" mapping.                                                                                                                                                                                                                                            |
| `mrm_lead_rationale_text`        | `""`                                          | String storing the MRM Lead's plain-English rationale for the tiering decision. This is initially populated from `tiering_results['mrm_lead_rationale_plain_english']` but can be edited by the user via `st.text_area` on the "Export Reports" page. The updated value is used in downloaded reports.                                                                                                                            |
| `download_json_tiering_content`  | `""`                                          | String content of the `risk_tiering.json` report, generated dynamically for `st.download_button`.                                                                                                                                                                                                                                                                                                                                     |
| `download_json_controls_content` | `""`                                          | String content of the `required_controls_checklist.json` report, generated dynamically for `st.download_button`.                                                                                                                                                                                                                                                                                                                    |
| `download_md_summary_content`    | `""`                                          | String content of the `executive_summary.md` report, generated dynamically for `st.download_button`.                                                                                                                                                                                                                                                                                                                                  |

### Application Structure and Flow

The Streamlit application will be a single `app.py` file using conditional rendering for a multi-page experience based on a sidebar `st.selectbox`.
...
from source import TIERING_LOGIC_VERSION, risk_factor_weights, tier_thresholds, control_expectations_library, \
                   apply_model_tiering_logic, map_controls_to_tier

# --- Session State Initialization ---
if 'current_page' not in st.session_state:
    st.session_state.current_page = "Ingest Model Submission"
...
with st.sidebar:
    ...
    # Manually create selectbox with disabled options
    selected_page_index = page_options.index(st.session_state.current_page)
    # Loop to render selectbox with enabled/disabled state
...
if st.session_state.current_page == "2) Official Tiering":
    if not st.session_state.model_registration_data:
        ...
        for tier, thresholds in tier_thresholds.items():
            st.markdown(f"**- {tier}**: Score $\ge$ {thresholds['min_score']} {'and <' + str(thresholds['max_score']) if thresholds['max_score'] != float('inf') else ''}")
        st.markdown(f"---")
...
if st.session_state.current_page == "3) Required Controls":
    if not st.session_state.tiering_results:
        ...
        if st.session_state.controls_checklist:
            ...
            # This requires inferring categories from control_name or control_id
